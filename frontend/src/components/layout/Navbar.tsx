  ...
  return (
    <>
      <header className="fixed top-4 left-0 right-0 z-40 flex justify-center px-4">
        <nav
          ...
          style={{
            ...
            boxShadow: '0 8px 32px rgba(0,0,0,0.60), 0 0 0 1px rgba(201,162,39,0.06) inset',
          }}
        >
          {/* Logo */}
          <Link to="/" style={{ fontFamily: "'Cinzel', serif", color: '#C9A227', fontSize: 18, fontWeight: 700, letterSpacing: 3, textDecoration: 'none' }}>
          ...
          <div className="hidden sm:flex items-center gap-1">
            {NAV_LINKS.map((l) => (
              <Link key={l.to} to={l.to}
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#C9A227'; (e.currentTarget as HTMLElement).style.background = 'rgba(201,162,39,0.10)'; }}
          ...
          <div className="flex items-center gap-3">
            ...
            {funded && (
              <button
                ...
                style={{ color: 'rgba(245,240,232,0.65)', background: 'rgba(201,162,39,0.07)', border: '1px solid rgba(201,162,39,0.12)' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = 'rgba(201,162,39,0.30)'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = 'rgba(201,162,39,0.12)'}
              >
            ...
            {user ? (
              <div className="relative hidden sm:block">
                ...
                {userMenu && (
                  <div className="absolute right-0 top-10 py-1 min-w-[140px] z-50"
                    style={{ background: 'rgba(8,8,12,0.98)', backdropFilter: 'blur(16px)', border: '1px solid rgba(201,162,39,0.18)', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
                    <button
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #E2C547, #C9A227)', color: '#0A0400', letterSpacing: 0.5 }}
              >
                Login
              </button>
            )}
            ...
            <button className="sm:hidden flex flex-col gap-1.5 p-2" onClick={() => setOpen(true)} aria-label="Open menu">
              {[0, 1, 2].map(i => (
                <span key={i} className="block rounded-full" style={{ width: 22, height: 2, background: '#C9A227' }} />
              ))}
            </button>
          </div>
        </nav>
      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between px-6 h-20" style={{ borderBottom: '1px solid rgba(201,162,39,0.12)' }}>
            <span style={{ fontFamily: "'Cinzel', serif", color: '#C9A227', fontSize: 18, fontWeight: 700, letterSpacing: 3 }}>
              JUZZ<span style={{ color: '#F5F0E8' }}>.BET</span>
            </span>
            <button onClick={() => setOpen(false)} style={{ color: '#C9A227', fontSize: 24, lineHeight: 1 }}>✕</button>
          </div>
          ...
          <div className="flex flex-col gap-1 p-6 mt-4">
            {NAV_LINKS.map((l) => (
              <Link key={l.to} to={l.to} onClick={() => setOpen(false)}
                ...
                style={{ color: 'rgba(245,240,232,0.75)', borderBottom: '1px solid rgba(201,162,39,0.08)', textDecoration: 'none' }}>
                {l.label}
              </Link>
            ))}
            ...
            {user ? (
              <>
                <button
                  ...
                  style={{ color: 'rgba(245,240,232,0.75)', borderBottom: '1px solid rgba(201,162,39,0.08)' }}>
                  Wallet · ${balance.toFixed(2)}
                </button>
                <button
                  onClick={() => { logout(); setOpen(false); }}
                  className="mt-6 text-center py-4 rounded-xl font-semibold text-lg"
                  style={{ border: '1px solid rgba(201,162,39,0.25)', color: 'rgba(245,240,232,0.5)' }}>
                  Sign out
                </button>
              </>
            ) : walletUser ? (
              ...
              <button
                ...
                className="mt-6 text-center py-4 rounded-xl font-semibold text-lg"
                style={{ background: 'linear-gradient(135deg, #E2C547, #C9A227)', color: '#0A0400' }}>
                Login
              </button>
            )}
          </div>
          ...
          <div className="flex-1 flex items-end justify-center pb-10">
            <p style={{ color: 'rgba(201,162,39,0.3)', fontSize: 11, letterSpacing: 4, textTransform: 'uppercase' }}>◈ AI Chess Prediction ◈</p>
          </div>
        </div>
      )}
