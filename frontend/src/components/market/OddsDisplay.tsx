    ...
    <>
      ...
      <div className="flex flex-col gap-2 lg:hidden">
        <div className="flex items-center gap-2">
          <span style={{ fontFamily: "'Inter', sans-serif", color: '#C9A227', fontSize: 11, fontWeight: 600, minWidth: 32 }}>
            {((a?.prob ?? 0) * 100).toFixed(0)}%
          </span>
          <div className="flex flex-1 rounded-full overflow-hidden h-2" style={{ background: 'rgba(201,162,39,0.12)' }}>
            {outcomes.map(o => (
              <motion.div
                key={o.key}
                style={{ background: o.key === a?.key ? 'linear-gradient(90deg,#C9A227,#C9A227)' : 'rgba(201,162,39,0.35)' }}
          ...
          {outcomes.map(o => {
            ...
            return (
              <motion.button
                ...
                style={{
                  border: `1px solid ${isSelected ? 'rgba(201,162,39,0.80)' : 'rgba(201,162,39,0.22)'}`,
                  background: isSelected ? 'rgba(201,162,39,0.18)' : 'rgba(0,0,0,0.25)',
                  borderRadius: 2,
                  boxShadow: isSelected ? '0 0 18px rgba(201,162,39,0.28)' : 'none',
                }}
              ...
              >
                <div className="flex items-center gap-1">
                  ...
                  <span style={{ fontFamily: "'Cinzel', serif", color: '#E8D4A0', fontSize: 14, fontWeight: 600, letterSpacing: 0.5, lineHeight: 1.2 }}>{o.label}</span>
                </div>
                <motion.span
                  key={odd}
                  style={{ fontFamily: "'Cinzel', serif", color: '#E2C547', fontSize: 18, fontWeight: 700, textShadow: isSelected ? '0 0 12px rgba(201,162,39,0.8)' : 'none' }}
                  initial={{ scale: 1.15 }}
                  animate={{ scale: 1 }}
                >
                  ×{odd.toFixed(2)}
        ...
        {outcomes.map(o => {
          ...
          return (
            <motion.button
              ...
              style={{
                border: `1px solid ${isSelected ? 'rgba(201,162,39,0.80)' : 'rgba(201,162,39,0.22)'}`,
                background: isSelected ? 'rgba(201,162,39,0.14)' : 'rgba(0,0,0,0.28)',
                borderRadius: 2,
                boxShadow: isSelected ? '0 0 20px rgba(201,162,39,0.25), inset 0 0 20px rgba(201,162,39,0.06)' : 'none',
              }}
            ...
            >
              ...
              <motion.div
                ...
                style={{ background: 'linear-gradient(90deg,#C9A227,#C9A227)', borderRadius: 2 }}
              ...
              <div className="relative flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div style={{ fontFamily: "'Cinzel', serif", color: '#E8D4A0', fontSize: 15, fontWeight: 600, letterSpacing: 0.5 }}>{o.label}</div>
                ...
                <div className="text-right">
                  <motion.div
                    key={odd}
                    style={{ fontFamily: "'Cinzel', serif", color: '#E2C547', fontSize: 22, fontWeight: 700, textShadow: isSelected ? '0 0 12px rgba(201,162,39,0.8)' : '0 0 8px rgba(201,162,39,0.4)' }}
                    initial={{ scale: 1.1 }}
                    animate={{ scale: 1 }}
                  >
                    ×{odd.toFixed(2)}
