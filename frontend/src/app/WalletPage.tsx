              ...
              <div className="flex flex-col gap-2">
                {[...bets].reverse().map(b => (
                  <div key={b.id} className="flex justify-between items-center py-2 border-b border-border last:border-0">
                    <div>
                      <span className="text-ivory text-sm">{b.pick}</span>
                      {b.matchLabel && <span className="text-muted text-xs ml-2">{b.matchLabel}</span>}
                      {b.settled && (
                        b.won
                          ? <span className="text-green-400 text-xs ml-2">won ${(b.stake * b.impliedOdds).toFixed(2)}</span>
                          : <span className="text-red-400 text-xs ml-2">lost</span>
                      )}
                      {!b.settled && <span className="text-gold text-xs ml-2">live</span>}
                    </div>
